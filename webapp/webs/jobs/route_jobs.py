...
from webapp.db.repository.jobs import create_new_job
from webapp.db.repository.jobs import list_jobs
from webapp.db.repository.jobs import retreive_job
from webapp.db.repository.jobs import search_job
from webapp.db.session import get_db
from webapp.schemas.jobs import JobCreate
from webapp.webs.jobs.forms import JobCreateForm
...
@router.get("/jobboard/")
async def jobboard(request: Request, db: Session = Depends(get_db), msg: str = None):
    jobs = list_jobs(db=db)
    return templates.TemplateResponse(
        "jobs/jobboard.html", {"request": request, "jobs": jobs, "msg": msg}
    )
...
def create_job_get(request: Request, db: Session = Depends(get_db)):
    return templates.TemplateResponse("jobs/create_job.html", {"request": request})


@router.post("/post-a-job/")
async def create_job_post(request: Request, db: Session = Depends(get_db)):
...
def search(
    ...
    return templates.TemplateResponse(
        "jobs/jobboard.html", {"request": request, "jobs": jobs}
    )
